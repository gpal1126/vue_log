<template>
    <div>
        ...
        <form @submit.prevent="onSubmitForm">
            <input ref="answer" v-model="value" minlength="4" maxlength="4" />
...
<script>
    ...
    const getNumbers = () => {
        ...
        const array = [];
        for(let i=0; i<4; i++){
            const chosen = candiates.splice(Math.floor(Math.random() * (9-i)), 1)[0];
            array.push(chosen);
        }
        return array;
    }
            ...
            onSubmitForm(e) {
                ...
                if( this.value === this.answer.join('') ){  //정답 맞췄으면
                    ...
                    if( this.tries.length >= 9 ){   //10번째 시도
                        ...
                        let strike = 0;
                        let ball = 0;
                        const answerArray = this.value.split('').map( v => parseInt(v) );   //문자열 -> 숫자배열로 변경
                        for(let i=0; i<4; i++){
